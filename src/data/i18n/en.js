export default {
  meta: {
    title: "Antonio Mata Marín — Technical Artist",
  },
  nav: {
    about: "About",
    experience: "Experience",
    education: "Education",
    portfolio: "Portfolio",
    articles: "Articles",
    recommendations: "Recommendations",
    skills: "Skills",
    contact: "Contact",
    langToggle: "ES",
    langLabel: "Choose language",
    backgroundLabel: "3D background",
    backgroundOn: "Disable 3D background for better performance",
    backgroundOff: "Enable 3D background",
    langTitle: {
      en: "View in English",
      es: "Ver en español",
    },
  },
  hero: {
    kicker: "Technical Artist",
    name: "Antonio Mata Marín",
    tagline:
      "Technical Artist specialized in improving real-time simulation workflows through tools, procedural systems, automation, and rendering technologies.",
    cta: "View Portfolio",
    location: "Sevilla, Spain",
    focusLabel: "Selected focus",
    focusAreas: ["Artist-facing tools", "Real-time rendering", "Procedural systems"],
  },
  about: {
    heading: "About Me",
    body:
      "I'm a Technical Artist working at the intersection of art and engineering, building procedural tools, shaders, and automation for real-time simulation. During my time at CARLA Simulator, I developed artist-facing tooling for the Digital Twins project, rendering systems for the AeroSim aerospace simulator, and optimized shaders that kept complex scenes running smoothly across platforms.\n\nOutside of work, I explore rendering techniques on my own — raymarched SDF scenes, non-photorealistic shading, and procedural UV systems — to keep growing as both an artist and an engineer.",
  },
  experience: {
    heading: "Work Experience",
    kicker: "Professional journey",
    intro: "Technical art work spanning production tooling, procedural content, simulation interfaces, and real-time rendering.",
    impact: "Selected responsibilities and impact",
  },
  education: {
    heading: "Education",
  },
  portfolio: {
    heading: "Portfolio",
    subheading: "A selection of tools, shaders, and procedural systems I've built.",
    filterAll: "All",
    filterFeatured: "My Best Projects",
    filterWork: "Work",
    filterPersonal: "Personal",
    workLabel: "Professional",
    personalLabel: "Personal",
    viewProject: "View project",
    noMedia: "No media yet for this project — check back soon.",
  },
  articles: {
    heading: "Articles",
    kicker: "Writing",
    subheading: "Notes on the maths, rendering, and tooling I work with — explained through the examples I actually build.",
    label: "Article",
    readingUnit: "min read",
    readArticle: "Read article",
  },
  articleDetail: {
    back: "Back to Articles",
    source: "Source code",
    sourceNote: "The examples in this article are available as a project on GitHub.",
    openRepo: "Open on GitHub",
    notFound: "Article not found.",
  },
  recommendations: {
    heading: "Recommendations",
    kicker: "Professional references",
    subheading: "First-hand accounts from the people who have managed, collaborated with, and studied alongside me.",
    formalLetter: "Formal recommendation",
    linkedin: "LinkedIn recommendation",
    readRecommendation: "Read full recommendation",
  },
  recommendationDetail: {
    back: "Back to Recommendations",
    date: "Date",
    context: "Context",
    language: "Original language",
    source: "Original source",
    sourcePdf: "Signed recommendation letter supplied as a PDF.",
    sourceLinkedIn: "Recommendation published on Antonio's LinkedIn profile.",
    openPdf: "Open original PDF",
    openLinkedIn: "View LinkedIn profile",
    notFound: "Recommendation not found.",
  },
  projectDetail: {
    back: "Back to Portfolio",
    role: "Role",
    techUsed: "Technology",
    gallery: "Gallery",
    demoVideo: "Demo Video",
    notFound: "Project not found.",
    notFoundBody: "We couldn't find that project. It may have been renamed or removed.",
  },
  caseStudy: {
    professional: "Professional project",
    personal: "Personal project",
    viewMedia: "View project media",
    overviewEyebrow: "Project foundation",
    overview: "Overview",
    projectFacts: "Project facts",
    timeline: "Timeline",
    focus: "Focus",
    process: "Project process",
    challenge: "The challenge",
    approach: "The approach",
    outcome: "The outcome",
    inMotion: "Workflow in motion",
    demo: "Project demo",
    demoIntro: "A closer look at the tool, effect, and real-time workflow.",
    detailsEyebrow: "Design decisions",
    highlights: "What makes it work",
    visualRecord: "Visual record",
    gallery: "Project gallery",
    galleryIntro: "Selected views from the final system and its authoring workflow.",
    image: "image",
    takeaway: "Final takeaway",
  },
  skills: {
    heading: "Technical Skills",
    kicker: "Capabilities",
    intro: "A production-focused toolkit connecting code, rendering, procedural content, and artist workflows.",
    technicalHeading: "Technical toolkit",
    softHeading: "Soft Skills",
    softIntro: "How I collaborate, communicate, and take ownership inside multidisciplinary teams.",
    languagesHeading: "Languages",
    languagesIntro: "Languages I use in professional and day-to-day collaboration.",
  },
  contact: {
    heading: "Contact",
    subheading: "Interested in working together, or just want to say hello? Reach out.",
    kicker: "Let's work together",
    email: "Email",
    phone: "Phone",
    emailCta: "The best way to reach me",
    emailNote: "Email me directly and I'll get back to you as soon as I can.",
    directHeading: "Other ways to connect",
    quick: {
      heading: "Quick contact",
      note: "Drop me a quick note right here — a subject and a short message, straight to my inbox.",
      name: "Your name",
      namePlaceholder: "Jane Doe",
      email: "Your email",
      emailPlaceholder: "you@studio.com",
      subject: "Subject",
      subjectPlaceholder: "What's this about?",
      message: "Message",
      messagePlaceholder: "Tell me about the role, project, or question.",
      detailed: "Something longer, or need to send files? Write to me directly at ",
      send: "Send message",
      sending: "Sending…",
      success: "Thanks — your message is on its way. I'll get back to you as soon as I can.",
      error: "Something went wrong sending your message. Please email me directly instead.",
      errorRequired: "Please fill in every field before sending.",
      errorEmail: "Please enter a valid email address so I can reply.",
      unconfigured: "The quick-contact form isn't set up yet. In the meantime, email me directly:",
    },
    cvHeading: "Resume / CV",
    cvNote: "A concise overview of my experience, tools, and technical art work.",
    downloadCv: "Download CV (EN)",
  },
  footer: {
    rights: "All rights reserved.",
  },
};
