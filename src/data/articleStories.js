  ...
  "cross-product": {
    en: {
      ...
      blocks: [
        ...
        {
          type: "code",
          label: "C# · EditorRightLeftExample.cs",
          code: [
        ...
        {
          type: "p",
          text: "With Unity's usual X-right, Y-up, Z-forward basis and exactly that operand order, a positive `endY.y` means right and a negative one means left. Swap the inputs and the meaning swaps too. Reading `.y` is equivalent here to measuring the cross-product result along world up; the repository keeps the demo direct and uses that sign to update the text.",
        },
        {
          type: "note",
          label: "What zero means",
          text: "The repository returns when `endY.y == 0`: the target is on the forward/back axis, so there is no left or right. In production, a small tolerance is safer than exact equality because floating-point values can hover just above or below zero. If front versus back matters too, that is a separate alignment check.",
        },
        ...
        {
          type: "code",
          label: "C# · CameraOrientationExample.cs",
          code: [
        ...
        },
        {
          ...
          text: "If the camera looks straight along `WorldUp`, forward and reference up become parallel. Their cross product is zero, right has no direction and the basis collapses. The repository catches this before normalising with `cross.sqrMagnitude > 0.0001f`; if the orientation is invalid, it clears `Right` and `Up` and returns. Also note the real source order: `Cross(Forward, WorldUp)` would point to the opposite side in this convention.",
        },
    ...
    es: {
      ...
      blocks: [
        ...
        {
          type: "p",
          text: "Con la base habitual de Unity — X derecha, Y arriba y Z frontal — y exactamente ese orden de inputs, un `endY.y` positivo significa derecha y uno negativo izquierda. Si intercambias los inputs, intercambias también el significado. Leer `.y` equivale aquí a medir el resultado del cross sobre el up global; el repositorio mantiene la demo directa y usa ese signo para actualizar el texto.",
        },
        {
          type: "note",
          label: "Qué significa cero",
          text: "El repositorio sale de la función cuando `endY.y == 0`: el objetivo está sobre el eje frontal/trasero, así que no hay izquierda ni derecha. En producción conviene usar una pequeña tolerancia en vez de una igualdad exacta, porque un float puede quedarse justo por encima o por debajo de cero. Distinguir delante de detrás sería otra comprobación de alineamiento.",
        },
        ...
        },
        {
          ...
          text: "Si la cámara mira justo en la dirección de `WorldUp`, forward y ese up de referencia se vuelven paralelos. El cross da cero, right se queda sin dirección y toda la base colapsa. El repositorio lo detecta antes de normalizar con `cross.sqrMagnitude > 0.0001f`; si la orientación no es válida, pone `Right` y `Up` a cero y sale. Fíjate también en el orden real del código: `Cross(Forward, WorldUp)` apuntaría al lado opuesto con esta convención.",
        },
